...
=====================================================
Uploads raw CSV data to HDFS and converts cleaned data to Parquet format
...
def upload_raw_via_cli(local_dir: str, hdfs_base: str):
    ...
    for filename in DATA_FILES:
        ...
        subprocess.run(["hadoop", "fs", "-rm", "-f", hdfs_path], capture_output=True)

        logger.info(f"Uploading {local_path} -> {hdfs_path}")
        subprocess.run(["hadoop", "fs", "-put", local_path, hdfs_path], check=True)
...
def clean_and_write_parquet(local_dir: str, hdfs_base: str):
    ...
    if os.path.exists(stores_path):
        ...
        logger.info(f"stores: {df_stores.count()} rows -> {cleaned_dir}/stores")
    ...
    if os.path.exists(features_path):
        ...
        df_features = df_features.withColumn(
            "Date",
            F.when(F.col("Date").isNull(), F.to_date(F.col("Date"), "yyyy-MM-dd"))
        ...
        for i in range(1, 6):
            ...
            df_features = df_features.withColumn(
                flag_name,
                F.when(
                    (F.col(col_name).isNotNull()) & (F.col("Date") >= F.lit(markdown_cutoff)),
                    F.lit(1)
                ).otherwise(F.lit(0))
            )
            ...
            df_features = df_features.withColumn(
                col_name,
                F.when(F.col(col_name).isNull(), F.lit(0.0)).otherwise(F.col(col_name))
            )

        df_features.write.mode("overwrite").parquet(f"{cleaned_dir}/features")
        logger.info(f"features: {df_features.count()} rows -> {cleaned_dir}/features")

    # --- train.csv ---
    train_path = os.path.join(local_dir, "train.csv")
    if os.path.exists(train_path):
        df_train = spark.read.csv(train_path, header=True, inferSchema=True)
        df_train = df_train.withColumn("Date", F.to_date(F.col("Date"), "dd/MM/yyyy"))
    ...
    if os.path.exists(test_path):
        ...
        df_test = df_test.withColumn("Date", F.to_date(F.col("Date"), "dd/MM/yyyy"))
...
def verify_hdfs_structure(hdfs_base: str):
    ...
    if result.returncode == 0:
        ...
        logger.warning("Could not list HDFS directory (hadoop CLI may not be available)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Upload Walmart sales data to HDFS (raw CSV + optional cleaned Parquet)"
    )
    ...
    parser.add_argument("--method", choices=["cli", "webhdfs"], default="cli",
                        help="Upload method for raw CSVs (default: cli)")
    ...
    if not args.skip_upload:
        if args.method == "cli":
            ...
            upload_raw_via_webhdfs(args.local_dir, args.hdfs_base, args.hdfs_host, args.hdfs_port)
    else:
